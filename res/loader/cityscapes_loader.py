...
def label_name_from_image_name(image_name):
    label_name = image_name.replace('leftImg8bit','gtFine_labelIds')
    return label_name

def format_label(imarray):
    '''
    Cityscapes labels are already label IDs, so no formatting was needed. Just \
    picking the first channel as it gets loaded as RGB image otherwise.
    '''
    return imarray[0,:,:]
...
def make_dataset(dir, extensions):
    ...
    for image_file in sorted(os.listdir(image_folder)):
        label_file = label_name_from_image_name(image_file)
