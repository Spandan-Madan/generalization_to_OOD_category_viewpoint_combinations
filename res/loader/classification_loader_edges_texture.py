...
#     print(image_array.shape)
    ...
    edges = np.clip(feature.canny(np.array(image_array_copy.detach().cpu())[0,:,:],sigma=1).astype('uint8'),0,255)
    edges_3 = np.stack([edges,edges,edges])
    mean = np.array([0.485, 0.456, 0.406])
    std = np.array([0.229, 0.224, 0.225])

    edges_normed = np.zeros((3,224,224))
    for i in range(3):
        edges_normed[i,:,:] = (edges - mean[i].item())/std[i].item()
    # edges_normed = (edges_3 - mean)/std
    xs,ys = np.where(car_mask==1)
    for i in range(len(xs)):
        x,y = xs[i],ys[i]
        image_array_copy[0,x,y] = edges_normed[0,x,y]*1.0
        image_array_copy[1,x,y] = edges_normed[1,x,y]*1.0
        image_array_copy[2,x,y] = edges_normed[2,x,y]*1.0
