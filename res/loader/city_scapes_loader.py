...
class cityscapesLoader(data.Dataset):
    ...
    def __init__(
        ...
        self.files[split] = recursive_glob(rootdir=self.images_base, suffix=".exr")
    ...
    def __getitem__(self, index):
        ...
        lbl_path = os.path.join(
            ...
            "label_"+ os.path.basename(img_path)[:-4] + ".png",
        )
...
#        img = m.imread(img)
        ...
        #print('path worked',img_path,lbl_path)
        return img, lbl
    ...
    def transform(self, img, lbl):
        ...
        """
        #try:
            #print('success before',img.shape)
        #img = m.imresize(
        #   img, (self.img_size[0], self.img_size[1]), mode='F'
        #)  # uint8 with RGB modei
        img = m.imresize(img,(512,1024))
            #print('success',img.shape)
            #print('failed',img.shape)
            #plt.imshow(img[:,:,0])
            #plt.show()
            #pass
        img = img[:, :, ::-1]  # RGB -> BGR
...
#        print(np.unique(lbl[lbl != self.ignore_index]),self.n_classes)
...
if __name__ == "__main__":
    ...
    local_path = "/data/graphics/toyota-pytorch/image_output_helen/Volvo/"
